 ...
 */
if (!class_exists('nK_Helper_Updater')) :
    ...
    */
    ...
    public function get_latest_plugin_version () {
        if(isset($this->latest_plugin_version)) {
            ...
            // Check cache
            $lastVersion = nk_theme()->get_cache('nk_theme_version_latest');
            ...
            $response = wp_remote_get('http://wp.nkdev.info/_api/?item_id=' . nk_theme()->plugin_name_sanitized . '&type=version');
    ...
    public function get_plugin_download_url () {
        if(isset($this->plugin_download_uri)) {
            ...
            $response = wp_remote_get('http://wp.nkdev.info/_api/?item_id=' . nk_theme()->plugin_name_sanitized . '&type=get-wp-uri');
            if(is_array($response) && $response['response']['code'] == 200 && $response['body']) {
                $response = json_decode(wp_remote_retrieve_body($response));
