...
function youplay_post_review_hexagon($small = false) {
	...
	return '<div class="youplay-hexagon-rating ' . ($small?'youplay-hexagon-rating-small':'') . '" data-max="' . intval($max_rating) . '" ' . ($small?'data-size="50"':'') . ' title="' . number_format($rating, 1, '.', '') . ' out of ' . intval($max_rating) . '" data-back-color="' . esc_attr($back_color) . '" data-front-color="' . esc_attr($front_color) . '" itemprop="aggregateRating" itemscope itemtype="http://schema.org/Rating"><meta itemprop="bestRating" content="' . intval($max_rating) . '"><span itemprop="ratingValue">' . number_format($rating, 1, '.', '') . '</span></div>';
}
endif;
